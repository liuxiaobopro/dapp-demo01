	...
	<div class="bg-dark/50 rounded-xl p-6 md:p-8 border border-primary/20 mb-10">
		<div class="flex flex-col md:flex-row justify-between items-center gap-6">
			...
			<div class="flex gap-4 w-full md:w-auto">
				...
				<button
					@click="$emit('test-balance')"
					:disabled="!isConnected"
					class="flex-1 md:flex-none bg-accent/20 text-accent py-3 px-6 rounded-lg font-medium border border-accent/30 hover:border-accent/60 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
				>
...
export default {
  ...
  emits: ['connect', 'disconnect', 'refresh', 'debug', 'test-balance']
}
</script>
